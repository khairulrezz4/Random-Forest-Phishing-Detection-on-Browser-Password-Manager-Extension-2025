...
# Add your extension origin here if needed (e.g. chrome-extension://<id>)
...
def predict_url():
    ...
    try:
        ...
        if MODEL_FEATURE_NAMES is not None:
            ...
            if extra:
                logger.debug(f"Dropping extra extractor columns (normal): {extra}")
...
@app.route("/")
def index():
    """Root index to aid scanners and quick manual checks."""
    return jsonify({
        "message": "Phishing detection API",
        "endpoints": ["/predict_url", "/predict_batch", "/health"]
    }), 200


@app.after_request
def set_security_headers(response):
    """Set basic security headers on all responses to reduce scanner warnings.

    Note: CORS is currently enabled via flask_cors.CORS(app). If you later
    want to restrict origins, prefer configuring CORS with specific origins
    (for example the dev server and your extension id) rather than using '*'.
    """
    response.headers['X-Content-Type-Options'] = 'nosniff'
    response.headers['X-Frame-Options'] = 'SAMEORIGIN'
    response.headers['Referrer-Policy'] = 'no-referrer-when-downgrade'
    # Add a stricter Content-Security-Policy for API responses to satisfy scanners
    # Include explicit directives that do not fallback to `default-src` to avoid ZAP warnings.
