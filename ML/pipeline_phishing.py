...
# ---------------------------------------------------------------------------
DATASET_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), "tarun_tiwari_dataset_balanced.csv")
RANDOM_STATE = 42
...
SUSPICIOUS_KEYWORDS = [
    ...
    "password", "wallet", "suspend", "unusual", "confirm", "expire", "urgent"
]

# Brand names commonly impersonated in phishing
BRAND_KEYWORDS = [
    "paypal", "amazon", "netflix", "apple", "microsoft", "google", "facebook",
    "instagram", "whatsapp", "telegram", "twitter", "linkedin", "dropbox",
    "coinbase", "metamask", "ledger", "trezor", "kraken", "binance", "crypto",
    "roblox", "steam", "ebay", "chase", "wellsfargo", "bankofamerica", "santander",
    "spotify", "adobe", "zoom", "docusign", "wetransfer", "outlook", "office365"
]

# Suspicious TLDs commonly used in phishing
SUSPICIOUS_TLDS = [
    "xyz", "top", "ml", "ga", "cf", "gq", "tk", "pw", "cc", "click", "link",
    "club", "work", "date", "loan", "men", "win", "bid", "trade", "webcam",
    "party", "review", "science", "racing", "accountant", "download", "stream",
    "cfd", "sbs", "icu", "buzz"
]

# Free hosting platforms often abused for phishing
FREE_HOSTING_PLATFORMS = [
    "github.io", "vercel.app", "netlify.app", "pages.dev", "herokuapp.com",
    "firebaseapp.com", "web.app", "azurewebsites.net", "cloudfront.net",
    "s3.amazonaws.com", "blogspot.com", "weebly.com", "wixsite.com", "godaddysites.com",
    "typedream.app", "framer.app", "framer.media", "framer.website", "webflow.io",
    "deno.dev", "workers.dev", "on-fleek.app", "netlify.com", "glitch.me",
    "repl.co", "stackblitz.io", "codesandbox.io", "github.dev", "gitpod.io",
    "render.com", "railway.app", "fly.dev", "duckdns.org", "ddns.net", "sslip.io"
]
...
def extract_url_features(url: str) -> dict:
    ...
    domain_part = no_scheme.split("/")[0].lower()
    ...
    # Check for brand impersonation in subdomain/path (not in actual domain)
    url_lower = raw.lower()
    brand_in_subdomain = 0
    brand_in_path = 0
    for brand in BRAND_KEYWORDS:
        # Check if brand appears in subdomain but domain is different (impersonation)
        if brand in domain_part:
            # If domain contains brand but is hosted on free platform = suspicious
            for platform in FREE_HOSTING_PLATFORMS:
                if platform in domain_part:
                    brand_in_subdomain = 1
                    break
        if brand in path_part.lower():
            brand_in_path = 1
    
    # Check for suspicious TLD
    suspicious_tld = 1 if tld in SUSPICIOUS_TLDS else 0
    
    # Check for free hosting platform
    on_free_hosting = 0
    for platform in FREE_HOSTING_PLATFORMS:
        if platform in domain_part:
            on_free_hosting = 1
    ...
    # Check for typosquatting patterns (common misspellings)
    typosquat_score = 0
    typosquat_patterns = [
        ("robiox", "roblox"), ("netfilx", "netflix"), ("amaz0n", "amazon"),
        ("paypa1", "paypal"), ("g00gle", "google"), ("micros0ft", "microsoft"),
        ("faceb00k", "facebook"), ("0utlook", "outlook"), ("app1e", "apple"),
        ("coinbas", "coinbase"), ("metamsk", "metamask"), ("trezo", "trezor"),
        ("ledgr", "ledger"), ("whatsap", "whatsapp"), ("instgram", "instagram"),
        ("linkdin", "linkedin"), ("tw1tter", "twitter"), ("dr0pbox", "dropbox"),
        ("walliet", "wallet"), ("ladger", "ledger"), ("trezr", "trezor"),
        ("krakn", "kraken"), ("binace", "binance"), ("uphld", "uphold"),
        ("robinhood", "robinhoodinvesting")  # legitimate vs fake
    ]
    for typo, _ in typosquat_patterns:
        if typo in url_lower:
            typosquat_score += 1
    
    # Check for IP-like patterns in domain (e.g., 192-168-1-1 or similar)
    ip_like_pattern = 1 if re.search(r'\d{1,3}[-_.]\d{1,3}[-_.]\d{1,3}[-_.]\d{1,3}', domain_part) else 0
    
    # Check for excessive hyphens (common in phishing)
    hyphen_count_domain = domain_part.count('-')
    excessive_hyphens = 1 if hyphen_count_domain >= 3 else 0
    
    # Check for login/auth related paths
    auth_path_keywords = ['login', 'signin', 'auth', 'verify', 'confirm', 'secure', 'account', 'update', 'password', 'credential']
    auth_in_path = sum(1 for kw in auth_path_keywords if kw in path_part.lower())
    
    # Check for random-looking strings (high entropy subdomains)
    subdomain = '.'.join(domain_tokens[:-2]) if len(domain_tokens) > 2 else ""
    subdomain_entropy = shannon_entropy(subdomain) if subdomain else 0
    
    # Check for numeric domain patterns (e.g., b45077.com)
    numeric_domain = 1 if re.match(r'^[a-z]?\d{4,}\.', domain_part) else 0
    
    # Check for clone/fake indicators in URL
    clone_keywords = ['clone', 'fake', 'test', 'demo', 'sample', 'phishing', 'scam']
    has_clone_keyword = sum(1 for kw in clone_keywords if kw in url_lower)
    ...
    feats = {
        ...
        # NEW FEATURES for modern phishing detection
        "suspicious_tld": suspicious_tld,
        "on_free_hosting": on_free_hosting,
        "brand_in_subdomain": brand_in_subdomain,
        "brand_in_path": brand_in_path,
        "typosquat_score": typosquat_score,
        "ip_like_pattern": ip_like_pattern,
        "excessive_hyphens": excessive_hyphens,
        "auth_in_path": auth_in_path,
        "subdomain_entropy": subdomain_entropy,
        "numeric_domain": numeric_domain,
        "has_clone_keyword": has_clone_keyword,
        "domain_hyphen_count": hyphen_count_domain,
        "path_depth": path_part.count('/'),
        "has_at_symbol": int('@' in raw),
        "uses_http": int(raw.lower().startswith('http://')),
        "double_slash_redirect": int('//' in raw[8:]),  # after scheme
    }
    return feats
...
def tree_based_selection(df: pd.DataFrame, label_col: str = "label") -> Tuple[pd.DataFrame, List[Tuple[str, float]]]:
    """Select features using tree-based importance, but ALWAYS keep critical phishing features.
    
    Critical features that detect modern phishing patterns are forced to stay regardless of 
    their importance score, because the training dataset may not contain enough examples
    of these patterns to learn their value.
    """
    # Critical features that MUST be included for modern phishing detection
    CRITICAL_FEATURES = {
        'on_free_hosting',      # Detects free hosting platforms (vercel, github.io, etc.)
        'brand_in_subdomain',   # Detects brand impersonation in subdomain
        'typosquat_score',      # Detects typosquatting patterns
        'suspicious_tld',       # Detects suspicious TLDs (.xyz, .top, .click, etc.)
        'has_clone_keyword',    # Detects clone/phishing keywords in URL
        'brand_in_path',        # Detects brand names in path
        'uses_http',            # Detects non-HTTPS usage (unusual for legitimate sites)
        'ip_like_pattern',      # Detects IP-like patterns in domain
        'excessive_hyphens',    # Detects excessive hyphen usage (phishing indicator)
        'numeric_domain',       # Detects heavily numeric domains
        'contains_login_token', # Detects login/signin keywords
        'auth_in_path',         # Detects authentication paths
    }
    ...
    # Start with features that pass the threshold
    keep = set([name for name, score in importances if score >= threshold])
    
    # ALWAYS add critical features even if they didn't pass the threshold
    available_critical = CRITICAL_FEATURES.intersection(set(feature_cols))
    forced_features = available_critical - keep  # Features we're forcing to include
    keep = keep.union(available_critical)
    
    if forced_features:
        print(f"  → Forced inclusion of {len(forced_features)} critical phishing features: {forced_features}")
    
    # Sort keep list by importance for cleaner output
    keep_list = sorted(list(keep), key=lambda x: dict(importances).get(x, 0), reverse=True)
    
    selected_df = df[["url", label_col] + keep_list].copy()
...
# Evaluation & Plots
# ---------------------------------------------------------------------------
def evaluate(model: RandomForestClassifier, X_train, y_train, X_val, y_val, X_test, y_test, feature_names: List[str]):
    y_pred_val = model.predict(X_val)
    y_pred_test = model.predict(X_test)
    y_prob_test = model.predict_proba(X_test)[:, 1]

    # Also print training set metrics for reference
    ...
    # Print a classification report for quick console inspection
